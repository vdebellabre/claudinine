...
| path stability | keyed by **project**; `~/.claude/projects/<slug>/` is durable and derivable | keyed by **session**: level 1 stable ≥ 4 months, level 2 varies, level 3 per session — and level 3's *name* has drifted (`local_`/`local_ditto_`, uuid matching nothing above it as of CC 2.1.234). **No path is both stable and derivable** — the structural reason baked absolute paths fail here. Still true at 2.1.234: the short fixed `<slug>` (`CLAUDE_CODE_PROJECT_DIR_NAME`) is gated on `3p` sessions, so first-party Cowork keeps the mangled slug — see cowork-compatibility.md D9 |
