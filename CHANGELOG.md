...
- **Microcompact boundaries reset the header-dedup segment.** The rule that keeps only one full retrieval header per boundary segment reset on `compact_boundary` only; a `microcompact_boundary` now also starts a fresh segment, pairing the two subtypes the way every other boundary check does. Preemptive: whether the loader slices context at a microcompact the same way is unconfirmed, but the cost of resetting is one extra full header — the cost of not resetting would be pointers at instructions the model can no longer see. Also documents (docs/session-file-changes.md) that chain-collapse deliberately drops removed records' `thinking` blocks from the digest: they are bulky, ephemeral, and signed; the mirror keeps them.
- **Reminder/document dedup skip the tail record.** A session killed mid-turn can leave the transcript ending on a record carrying a duplicate `<system-reminder>` or a ≥1KB duplicate block; deduping it tripped the rewrite layer's tail protection and silently discarded EVERY rule's work for the pass, repeatedly, until something was appended. Both rules now skip the file's final record (same tail guard as the other four rules) and converge on a later pass.
...
- **Manifest wording.** Plugin and marketplace descriptions now read "silent Claude context optimizer" — Claudinine serves Claude Desktop and Cowork sessions too, not only Claude Code. Also dependency bumps (Roslynator 4.16.1, TUnit 1.65.31) and the changelog added to the solution view.
